...
void moveCamera(Entity* camera, float radius) {
    ...
    if (shift) {
        ...
        auto forward = (centerDist > (farCamDist - nearCamDist) * 0.5f) ? camera->transform.up() : camera->transform.forward();
...
void adjustCamera(Entity* camera, vec3 positionDir, float dist) {
    ...
    auto forward = camera->transform.forward(); // don't need to do any conditional branching here because of the nlerp
    ...
    auto faceDir = glm::normalize(glm::mix(-positionDir, tangent, clampf((farCamDist - dist) / (farCamDist - nearCamDist), 0.f, 1.f)));
