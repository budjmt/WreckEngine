...
struct GLtexture {
	...
	inline void genMipMap() { GL_CHECK(glGenerateMipmap(type)); }
...
struct GLbuffer {
	...
	inline void bind() const { GL_CHECK(glBindBuffer(target, *buffer)); }
	inline void unbind()     { GL_CHECK(glBindBuffer(target, 0)); }
...
struct GLVAO {
	...
	inline void create() const  { if (*vao != def) return; GL_CHECK(glGenVertexArrays(1, vao.get())); }
	inline void bind() const    { GL_CHECK(glBindVertexArray(*vao)); }
	static inline void unbind() { GL_CHECK(glBindVertexArray(0)); }
};
...
struct GLprogram {
	...
	inline void use() const { GL_CHECK(glUseProgram(*program)); }
