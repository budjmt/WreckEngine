...
public:
    ...
    const AABB& aabb() { return transformed_aabb; }
    void updateDims();
    ...
    std::vector<vec3> clipPolyAgainstEdge(std::vector<vec3>& input, const vec3 sideNormal, const vec3 sideVert, const vec3 refNorm, const vec3 refCenter) const;
